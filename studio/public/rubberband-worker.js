...
(async () => {
  try {
    const wasmModule = await WebAssembly.compileStreaming(fetch('/studio/rubberband.wasm'));

    // Import the UMD bundle dynamically
    importScripts('/studio/rubberband.umd.min.js');
...
onmessage = async function(e) {
  ...
  try {
    // Convert stretchFactor to timeRatio (inverse relationship)
    const timeRatio = 1 / stretchFactor;
