...
export default(function ContextAudio() {
  let buffers = new Map(),
    ...
    _resumeContext = () => {
      return window.audioContextInstance.state === 'suspended' && window.audioContextInstance.resume()
        .then(() => {
          return true;
        })
        .catch(e => {
    ...
    _stopContext = () => {
      return window.audioContextInstance.state === 'running' && window.audioContextInstance.suspend()
        .then(() => {
          return true;
        })
        .catch(e => {
