...
const MAX_WAVEFORM_SAMPLES = 50000;
...
export default class Audio {
  constructor(config = {}) {
    ...
    this.buffer = new Buffer({
      ...
      startTimeBuffer: config.startTimeBuffer, endTimeBuffer: config.endTimeBuffer, isCutBuffer: config.isCutBuffer,
  ...
  releaseResources() {
    ...
    return ContextAudio.close().then(() => {
      this._volumeData = [];
  ...
  stretchBuffer(stretchFactor = 1, pitchValue) {
    return this.buffer.stretch(this.idTrack, stretchFactor, pitchValue)
      .then(res => {
        return res;
      })
      .catch(err => {
  ...
  /** Play audio */
  play() {
    return this.playing === false && this.buffer.get()
        .then(buff => {
  ...
  setVolumeData(sampleArrayInput) {
    return this.gainNode.setVolumeData(sampleArrayInput);
  }
  ...
  getWaveForm(scale) {
    ...
    return new Promise(resolve => {
      ...
      this.buffer.get().then((buff) => {
        ...
        const numSamples = duration / scale;
        ...
        resolve({ path: path, sampleRate: numSamples });
      });
    });
  }
}
