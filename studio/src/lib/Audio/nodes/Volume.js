export default class Volume {
  ...
  setGain(currentTime, pausedAt) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      curvePoints.length > 1 && this.node.gain.setValueCurveAtTime(curvePoints, curveStartTime, curveDurationTime);
      if (isPaddingZone === true && curve.startTime <= pausedAt && curve.startTime + curveDurationTime >= pausedAt) {
        isPaddingZone = false;
      }
      if (pausedAt > 0 && isPaddingZone === true && curve.startTime + curveDurationTime < pausedAt) {
  ...
  setVolumeData(sampleArrayInput) {
    let length = sampleArrayInput.length, sampleArrayOutput = [];
    for (let i = 0; i < length; i++) {
      let sample = sampleArrayInput[i];
      sampleArrayOutput.push({startTime: sample.startTime, durationTime: sample.durationTime, data: sample.data});
    }
