...
module.exports = flow = function(temporaryFolder) {
  ...
  try {
    if (!fs.existsSync($.temporaryFolder)) {
      ...
      console.log('Created flow temporary folder:', $.temporaryFolder);
    }
  } catch (e) {
    console.error('Error creating flow temporary folder:', e);
  }
  ...
  function validateRequest(chunkNumber, chunkSize, totalSize, identifier, filename, fileSize) {
    ...
    if (typeof(fileSize) != 'undefined') {
      ...
      // Last chunk size: remainder of totalSize / chunkSize, or chunkSize if evenly divisible
      var expectedLastChunkSize = totalSize % chunkSize || chunkSize;
      if (numberOfChunks > 1 && chunkNumber == numberOfChunks && fileSize != expectedLastChunkSize) {
        // The chunk in the POST is the last one, and the file is not the correct size
  ...
  $.post = function(req, callback) {
    ...
    var original_filename = files[$.fileParameterName] ? files[$.fileParameterName]['originalFilename'] : null;
    ...
    if (validation === 'valid') {
      ...
      // Save the chunk - use copyFile + unlink instead of rename to support cross-filesystem moves
      var sourcePath = files[$.fileParameterName].path;
      fs.copyFile(sourcePath, chunkFilename, function(err) {
        if (err) {
          console.error('Error copying chunk file:', err);
          callback('invalid_flow_request', null, null, null);
          return;
        }
        // Remove the original temp file
        fs.unlink(sourcePath, function(unlinkErr) {
          if (unlinkErr) {
            console.warn('Warning: could not remove temp file:', unlinkErr);
          }
        });
    ...
    var pipeChunkRm = function(number) {
      ...
      fs.exists(chunkFilename, function(exists) {
        if (exists) {
          ...
          //console.log('done ', options.onDone);
          if (options.onDone) options.onDone();
        }
      });
    };
